...
import { Card, Col, Container, Form, Nav, Row, Tab, TabContent, Table, TabPane, Tabs } from "react-bootstrap";
        ...
        <div>
            <Container>
                <Row>
                    <Col sm={12}>
                        <Card>
                            <Card.Header className="fs-2">{cve} 詳細</Card.Header>
                            <Card.Body>
                                <Card.Title className="fw-bold">内容</Card.Title>
                                <Card.Text>{cveItem?.cve?.description?.description_data[0]?.value}</Card.Text>
                            </Card.Body>
                            <Card.Body>
                                ...
                                <Tabs activeKey={cvssKey} id="uncontrolled-tab-example" onSelect={(k) => setCvssKey(k ?? "")}>
                                    <Tab eventKey="cvss3" title="CVSS 3">
                                        <TabContent className="border-start border-bottom border-end">
                                            <Card.Body>
                                                ...
                                                <Card.Text>
                                                    <span className="fw-bolder">Base Score:</span>
                                                    {cveItem?.impact?.baseMetricV3?.impactScore}
                                                </Card.Text>
                                                <Card.Text>
                                    ...
                                    <Tab eventKey="cvss2" title="CVSS 2">
                                        <TabContent className="border-start border-bottom border-end">
                                            <Card.Body>
                                                <Card.Title className="fw-bold">CVSS 2.0 Severity and Metrics</Card.Title>
                                                <Card.Text>
                                                    <span className="fw-bolder">Base Score:</span>
                            ...
                            <Card.Body>
                                ...
                                <Table striped bordered hover size="sm">
                                    ...
                                    <tbody>
                                        {cveItem?.cve?.references?.reference_data?.map((d) => (
                                            <tr key={d.url}>
                                                <td className="text-break">{d.url}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </Table>
                            ...
                            <Card.Body>
                                ...
                                <Card.Text>{cveItem?.publishedDate}</Card.Text>
                            </Card.Body>
                            <Card.Body>
                                <Card.Title className="fw-bold">最終更新日時</Card.Title>
                                <Card.Text>{cveItem?.lastModifiedDate}</Card.Text>
                            </Card.Body>
                        </Card>
                    </Col>
                </Row>
